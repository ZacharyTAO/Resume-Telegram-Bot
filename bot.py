...
TOKEN = '5046518658:AAGlBCM7YWszBCknuZBzRcdqqHCpX9izcvs'
...
def links(update: Update, context: CallbackContext) -> int:
    ...
    for i in range(num_links):
        keyboard[0].append(InlineKeyboardButton("Edit " + user_links[i]["link_description"], callback_data="LINK_"+user_links[i]["link_description"]))
...
def editlink(update: Update, context: CallbackContext):
    ...
    query = update.callback_query
    query.answer()
    link_description = data[5:]
    username = query.message.chat.username
    link_url = db.get_link_url(username, link_description)
    msg = "Your current link description is <b>" + link_description + "</b>\n" + \
    ...
    context.user_data['link_description'] = link_description
...
def edit_link_url(update: Update, context: CallbackContext):
    ...
    username = update.message.chat.username
    link_description = context.user_data['link_description']
    db.edit_link(username, link_description, link_url)
    msg = "Your link for <b>" + link_description + "</b> has been added."
    update.message.reply_text(text=msg,  parse_mode='html')
    return THIRD
###
